...
module Isucari
  class Web < Sinatra::Base
    ...
    PAYMENT_SERVICE_ISUCARI_APIKEY = "#{SERVICE_API_KEY}"
    ...
    helpers do
      def db
        Thread.current[:db] ||= Mysql2::Client.new(
          'host' => ENV['MYSQL_HOST'] || "#{MYSQL_HOST}",
          'port' => ENV['MYSQL_PORT'] || "#{MYSQL_PORT}",
      ...
      def get_users(user_list)
	      db.query("SELECT * FROM `users` WHERE id in (#{user_list.join(',')})")
          .map do |user|
            {
              'id' => user['id'],
	...
	category = db.xquery('SELECT `child`.*, `parent`.`category_name` AS `parent_name` from `categories` AS `child` LEFT JOIN `categories` AS `parent` ON `parent`.`id` = `child`.`parent_id` WHERE `child`.`id` = ?', category_id).first
    ...
    get '/new_items.json' do
      ...
      # seller + categoryu を上で取得して、ここで
      seller_list = get_users(items.map{|item| item['seller_id']}.compact)

      item_simples = items.map do |item|
        seller = seller_list.select{|seller| seller['id'] == item['seller_id']}.first
    ...
    get '/users/transactions.json' do
      ...
      user_list = get_users(items.map{|item| [item['seller_id'],item['buyer_id']]}.compact)
